...
function showDepartments(){
  ...
  departments[selectedStream].forEach(dep=>{
    ...
    b.textContent=dep;
    if(selectedDepartment && dep.toLowerCase()===selectedDepartment) b.classList.add('active');
    b.addEventListener('click',()=>{
      selectedDepartment=dep.toLowerCase();
      Array.from(departmentsDiv.children).forEach(btn=>btn.classList.remove('active'));
...
function showSemesters(){
  ...
  for(let i=1;i<=8;i++){
    ...
    if(selectedSemester && i===selectedSemester) b.classList.add('active');
    b.addEventListener('click',()=>{
      selectedSemester=i;
      Array.from(semestersDiv.children).forEach(btn=>btn.classList.remove('active'));
      b.classList.add('active');
      loadSubjects(); 
  ...
  try{
    ...
    subjects.forEach(s=>{
      ...
      ["S","A","B","C","D","E","F"].forEach(g=>{
        ...
        btn.addEventListener('click',()=>{
          grades[s.code]=g; Array.from(gradeDiv.children).forEach(b=>b.classList.remove('selected')); btn.classList.add('selected');
        });
...
// Saved Semester Page with colored cards
saveSemesterBtn.addEventListener('click',()=>{
  ...
  semesterGPAs.forEach((gpa,i)=>{
    if(gpa!==null){
      ...
      card.innerHTML = `<h3>Semester ${i+1}</h3><p>GPA: ${gpa}</p><p>Percentage: ${percentage}%</p>`;
      if(gpa>=9) card.style.background = 'linear-gradient(135deg, #00c853, #b2ff59)';
      else if(gpa>=7) card.style.background = 'linear-gradient(135deg, #ffeb3b, #ffc107)';
      else card.style.background = 'linear-gradient(135deg, #f44336, #ff8a80)';
      savedSemestersList.appendChild(card);
    }
  });
  showPage('saved-semesters-page');
});

// Add navigation to Grade Reference
const gradeRefBtn = document.getElementById('grade-ref-btn'); // if you have a button somewhere
if(gradeRefBtn){
  gradeRefBtn.addEventListener('click',()=> showPage('grade-reference-page'));
}