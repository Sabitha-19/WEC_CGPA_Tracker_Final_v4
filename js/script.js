...
// ===== State =====
let selectedStream = '', selectedDepartment = '', selectedSemester = 0, subjects = [], grades = {};
...
saveIcon.addEventListener('click', () => { showSavedSemesters(); showPage('saved-page'); });
...
calculateBtn.addEventListener('click',()=>{
  if(Object.keys(grades).length!==subjects.length){ alert("Please select all grades"); return; }
  ...
  encouragements.some(e=>{
    if(cgpa>=e.min){ encouragementDisplay.textContent=e.msg; return true; }
  });
...
deleteBtn.addEventListener('click',()=>{
  ...
  localStorage.setItem('semesterGPAs', JSON.stringify(semesterGPAs));
  subjectsList.innerHTML=''; gpaDisplay.textContent=''; cgpaDisplay.textContent=''; percentageDisplay.textContent=''; encouragementDisplay.textContent='';
  updateChart();
});
...
function updateChart(){ gpaChart.data.datasets