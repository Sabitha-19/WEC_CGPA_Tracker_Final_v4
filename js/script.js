...
function loadSubjects(){
  ...
  fetch(filePath)
    ...
    .then(data => {
      subjects = data.subjects || data; // handle if array or {subjects: []}
...
document.getElementById('calculate-btn').addEventListener('click', () => {
  ...
  subjects.forEach(sub => {
    ...
    totalPoints += sub.credits * gradePoints[grades[sub.code]];
  });

  const gpa = (totalPoints / totalCredits).toFixed(2);
  const cgpa = gpa; // single semester for now
  ...
  for(let e of encouragements){
    if(cgpa >= e.min){
      document.getElementById('encouragement').innerText = e.msg;
      break;
    }
  }
...
function saveSemester(){
  savedSemesters.push({
    ...
    gpa: document.getElementById('gpa-display').innerText.split(": ")[1]
  });
...
function openGraph(){
  ...
  const semesterData = Array(8).fill(null);
  savedSemesters.forEach(s => semesterData[s.semester-1] = parseFloat(s.gpa));
