let selectedStream="",selectedDepartment="",selectedSemester=0,subjects=[],grades={},savedSemesters=JSON.parse(localStorage.getItem("savedSemesters"))||[],semesterChart;
...
function loadSubjects(){
  ...
  fetch(file)
    .then(res=>{if(!res.ok)throw new Error("File missing");return res.json();})
    .then(data=>{
      subjects=data;
      list.innerHTML="";
      subjects.forEach(s=>{
        ...
        div.innerHTML=`<strong>${s.code}</strong><br>${s.name} (${s.credits} credits)
    ...
    }).catch(()=>alert("Subjects file missing:\n"+file));
}
...
document.getElementById("calculate-btn").onclick=()=>{
  ...
  for(let s of subjects){
    if(!grades[s.code]){alert("Please select all grades");return;}
    totalCredits+=s.credits;
    totalPoints+=s.credits*gradePoints[grades[s.code]];
  }
  const semesterGPA=totalPoints/totalCredits;
  savedSemesters.push({semester:selectedSemester,gpa:+semesterGPA.toFixed(2)});
...
function showSaved(){
  ...
  list.innerHTML="";
  if(savedSemesters.length===0){list.innerHTML="<p>No CGPA data saved</p>";return;}
